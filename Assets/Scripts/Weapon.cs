...
public class Weapon : MonoBehaviour
{
    ...
    public void Shoot()
    {
        ...
        if (CurrentClip > 0 && !IsReloading)
        {
            ...
            if (Physics.Raycast(Camera.main.ViewportPointToRay(new Vector3(0.5f, 0.5f, 0)), out shotHit, MaxRange, layer, QueryTriggerInteraction.Ignore))
            {
                ...
                else if (shotHit.collider.CompareTag("Switch"))
                {
                    shotHit.collider.GetComponent<PlatformSwitch>().Hit();
                }
            }
        }
        else
        {
            SoundManager.instance.PlayWithoutWait(EmptyClip);
        }
    }

    private void PlayFireEffect()
    {
        ShotClips = SoundManager.instance.RandomizeSoundFx(ShotClips);
    }
    ...
    private IEnumerator ReloadCoroutine()
    {
        ...
        else if (CurrentAmmo < MagazineSize)
        {
            ...
            CurrentAmmo = 0;
            SoundManager.instance.PlayWithoutWait(ReloadClip);
        }
        else
        {
